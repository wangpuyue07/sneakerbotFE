...
Persistence.prototype.createItem = function (command, options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        command.creatorId = command._userContext.id;
...
Persistence.prototype.listItems = function (command, options) {
    var that = this;
    command = command || {};
    command.orderBy = command.orderBy || 'createdAt';
...
Persistence.prototype.deleteItem = function (command, options) {
    var that = this;
    return that.sqlModel.find({where: {id: command.id, deleted: 0}}).then(res => {
        if (!res) throw new service.ValidationError('Cannot find the given item.');
        return res.updateAttributes({deleted: true}, ['deleted']).then(res => {
            events.emit(that.events.onDeleted, {id: command.id});
        })
    });
};
...
Persistence.prototype.changePriority = function (command) {
    ...
    return sql.db.query(itemQuery.toString(), {type: sql.db.QueryTypes.SELECT}).then(item => {
        ...
        var updateQuery = squel.update().table(tableName).set('priority', command.priority).where('id=?', command.id);
