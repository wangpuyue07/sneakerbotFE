...
exports.handleWith = function (handler, options) {
    ...
    return function (req, res) {
        ...
        command = _.merge(command, options.customParams);
        ...
        return new Promise(function (resolve) {
            ...
            if (options.setCookie && data.accessToken) {
                exports.setCookie(res, data.accessToken);
            }
...
exports.stripContext = function (command) {
    ...
    for (var key in command) {
        ...
        if (key.indexOf('_') !== 0) response[key] = command[key];
    }
    return response
};
...
exports.hydrateList = function (items, hydrateTargets, entityResolvers) {
    var excludedEntityTypes = [constants.entityTypes.comment];
    ...
    items.forEach(item => {
        hydrateTargets.forEach(target => {
            ...
            if (excludedEntityTypes.indexOf(type) === -1) {
                initIdGroups(type);
    ...
    return new Promise((resolve, reject) => {
        async.parallel(tasks, function (err, results) {
            ...
            for (var i in items) {
                hydrateTargets.forEach(target => {
                    items[i][target.hydratedField] = _.find(results, function (x) {
                        if (!x.id)
                            throw new Error('The target entityIdField:' + target.entityIdField +
                                ' does not have a corresponding id on the actual object.');
                        return x.id == items[i][target.entityIdField];
                    });
                })
            }
            resolve(items);
