...
var persistence = new Persistence(constants.entityTypes.feedback_stocks);
...
exports.createFeedback = function(command){
    var rules = {
        ...
        description_high: Joi.string(),
        description_low: Joi.string()
    };
    service.validateSync(command, rules);
    constants.stock_feedback_bundle[command.productId]=undefined;
...
exports.listFeedback = function (command) {
    return new Promise(function (resolve,reject) {
        if(constants.stock_feedback_bundle[command.productId]){
            resolve(constants.stock_feedback_bundle[command.productId])
        }else{
            persistence.listItems(command).then(res=>{
                constants.stock_feedback_bundle[command.productId]=res;
                resolve(res);
            })
        }
    });
...
exports.getFeedback = function (command) {
    return new Promise(function(resolve,reject){
        if(constants.stock_feedback[command.cid]){
            resolve(constants.stock_feedback[command.cid]);
        }else{
            persistence.getItem(command).then(res=>{
                constants.stock_feedback[command.cid]=res;
                resolve(res);
            })
        }
    });
