...
var persistence = new Persistence(constants.entityTypes.feedback_prices);
...
exports.createFeedback = function(command){
    ...
    constants.price_feedback_bundle[command.productId]=undefined;
    return persistence.createItem(command);
};
exports.getFeedback = function (command) {
    return new Promise(function(resolve,reject){
        if(constants.price_feedback[command.cid]){
            resolve(constants.price_feedback[command.cid]);
        }else{
            persistence.getItem(command).then(res=>{
                constants.price_feedback[command.cid]=res;
                resolve(res);
            })
        }
    });
...
exports.listFeedback = function (command) {
    return new Promise(function (resolve,reject) {
        if(constants.price_feedback_bundle[command.productId]){
            resolve(constants.price_feedback_bundle[command.productId])
        }else{
            persistence.listItems(command).then(res=>{
                constants.price_feedback_bundle[command.productId]=res;
                resolve(res);
            })
        }
    });
